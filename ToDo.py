...
class TodoApp(App):
    ...
    # ---------- GRAPH ----------
    def _render_tag_links(self, open_tasks, index_map):
        # Per-tag connector groups: each #tag fans lines out to every task with it.
        # Labels are never truncated — they wrap like the rest of the tree.
        p = self._palette()
        tagmap = {}
        for t in open_tasks:
    ...
    def build_graph(self):
        ...
        walk(tree)
        lines.extend(self._render_tag_links(open_tasks, index_map))
        return "\n".join(lines)
